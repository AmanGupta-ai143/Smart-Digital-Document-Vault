...
function DocumentCard({ doc, view, onOpen, onToggleFavorite }) {
  ...
  if (view === "list") {
    return (
      <button onClick={() => onOpen(doc._id)} className="w-full flex items-center gap-3 bg-white border border-slate-200 hover:border-teal-300 rounded-lg px-4 py-3 text-left transition-colors">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-slate-400">{doc.category} · {fmtDate(doc.createdAt)} · {fmtBytes(doc.fileSizeBytes)}</p>
        </div>
  ...
  return (
    <div className="bg-white border border-slate-200 hover:shadow-md hover:border-teal-200 rounded-xl overflow-hidden transition-all">
      ...
      <button onClick={() => onOpen(doc._id)} className="w-full text-left p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <span onClick={(e) => { e.stopPropagation(); onToggleFavorite(doc); }} className="p-1 rounded hover:bg-slate-100">
            <Star size={15} className={doc.isFavorite ? "text-amber-500 fill-amber-500" : "text-slate-300"} />
          </span>
        </div>
        <p className="text-sm font-medium text-slate-800 leading-snug line-clamp-2 mb-1.5">{doc.fileName}</p>
        <p className="text-xs text-slate-400 mb-3">{fmtDate(doc.createdAt)} · {fmtBytes(doc.fileSizeBytes)}</p>
  ...
  return (
    <div className="p-5 md:p-8 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="font-serif text-2xl text-slate-900">My Documents</h1>
          <p className="text-slate-500 text-sm mt-1">Organize, search, and manage everything in one place.</p>
        </div>
      ...
      <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`px-3.5 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${tab === t.key ? "bg-slate-900 text-white" : "bg-white border border-slate-200 text-slate-600 hover:border-slate-300"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 flex-1 min-w-[200px]">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search this list…" className="text-sm outline-none flex-1" />
        </div>
        <button onClick={() => setShowFilters((s) => !s)} className="flex items-center gap-1.5 border border-slate-200 bg-white rounded-lg px-3 py-2 text-sm text-slate-600">
          <Filter size={14} /> Filter
        </button>
        <div className="flex border border-slate-200 rounded-lg overflow-hidden">
          <button onClick={() => setView("grid")} className={`p-2 ${view === "grid" ? "bg-slate-900 text-white" : "bg-white text-slate-500"}`}><Grid3x3 size={15} /></button>
          <button onClick={() => setView("list")} className={`p-2 ${view === "list" ? "bg-slate-900 text-white" : "bg-white text-slate-500"}`}><ListIcon size={15} /></button>
        </div>
      </div>

      {showFilters && (
        <div className="flex flex-wrap gap-1.5 mb-5">
          <button onClick={() => setCategory("")} className={`text-xs px-2.5 py-1 rounded-full border ${category === "" ? "bg-teal-700 text-white border-teal-700" : "border-slate-200 text-slate-600"}`}>All</button>
          {DOC_CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCategory(c)} className={`text-xs px-2.5 py-1 rounded-full border ${category === c ? "bg-teal-700 text-white border-teal-700" : "border-slate-200 text-slate-600"}`}>{c}</button>
          ))}
        </div>
      )}
