...
export default function AIAssistant({ initialDocId }) {
  ...
  return (
    <div className="p-5 md:p-8 max-w-4xl mx-auto flex flex-col h-[calc(100vh-64px)]">
      <div className="mb-4">
        <h1 className="font-serif text-2xl text-slate-900">Ask DocMind AI</h1>
        <p className="text-slate-500 text-sm mt-1">Search, understand, and explore your documents using natural language.</p>
      </div>

      <div className="flex items-center gap-2 mb-4">
        <span className="text-xs text-slate-500">Scope:</span>
        <button onClick={() => setScope("all")} className={`text-xs px-2.5 py-1 rounded-full border ${scope === "all" ? "bg-slate-900 text-white border-slate-900" : "border-slate-200 text-slate-600"}`}>All Documents</button>
        <select value={scopeDocId} onChange={(e) => { setScopeDocId(e.target.value); setScope("one"); }} className={`text-xs px-2.5 py-1 rounded-full border outline-none ${scope === "one" ? "bg-slate-900 text-white border-slate-900" : "border-slate-200 text-slate-600"}`}>
          {documents.map((d) => <option key={d._id} value={d._id} className="text-slate-900">{d.fileName}</option>)}
        </select>
      </div>

      <div className="flex-1 overflow-y-auto bg-white border border-slate-200 rounded-xl p-5 space-y-5 mb-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] ${m.role === "user" ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-800"} rounded-2xl px-4 py-3`}>
              ...
              {m.suggestions?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {m.suggestions.map((s) => <button key={s} onClick={() => send(s)} className="text-xs bg-white border border-slate-200 hover:border-teal-400 text-slate-600 px-2.5 py-1 rounded-full">{s}</button>)}
                </div>
              )}
            </div>
          </div>
        ...
        {sending && (
          <div className="flex justify-start">
            <div className="bg-slate-50 rounded-2xl px-4 py-3 flex items-center gap-2 text-slate-400 text-sm">
      ...
      <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Ask a question…" className="flex-1 text-sm outline-none py-2" />
