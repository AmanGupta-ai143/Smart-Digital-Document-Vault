    ...
    <div className="min-h-screen bg-stone-50 font-sans grid md:grid-cols-2">
      <div className="hidden md:flex flex-col justify-between bg-slate-900 text-white p-10">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm max-w-xs">Your documents and contacts sync securely the moment you sign in — phone, laptop, or desktop.</p>
        </div>
    ...
    return (
      <AuthLayout>
        <div className="w-full max-w-sm">
          <button onClick={() => setTwoFAChallenge(null)} className="text-sm text-slate-400 hover:text-slate-600 flex items-center gap-1 mb-6"><ArrowLeft size={14} /> Back</button>
          <h1 className="font-serif text-2xl mb-1">Two-factor verification</h1>
  ...
  return (
    <AuthLayout>
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-slate-500 mb-6">Log in to access your vault.</p>

        {error && <div className="bg-rose-50 border border-rose-100 text-rose-700 text-sm rounded-lg px-3 py-2 mb-4">{error}</div>}

        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="text-xs font-medium text-slate-600 block mb-1.5">Email or User ID</label>
            <input
              type="email" required value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
              className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent"
            />
          </div>
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent pr-10"
              />
        ...
        <p className="text-sm text-slate-500 text-center mt-6">Don't have an account? <button onClick={onGoSignup} className="text-teal-700 font-medium">Create Account</button></p>
        <p className="text-xs text-slate-400 text-center mt-4">Your personal vault is protected and accessible only through authorized accounts and devices.</p>
      </div>
    </AuthLayout>
  );
}
