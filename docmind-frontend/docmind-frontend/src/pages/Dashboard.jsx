...
export default function Dashboard({ setPage, openDoc, openUpload }) {
  ...
  return (
    <div className="p-5 md:p-8 max-w-6xl mx-auto space-y-8">
      <div>
        <h1 className="font-serif text-2xl md:text-3xl text-slate-900">Good morning, {firstName} 👋</h1>
        <p className="text-slate-500 text-sm mt-1">Here's what's happening in your digital vault.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <p className="text-2xl font-serif text-slate-900">{c.value}</p>
      ...
      <div className="grid sm:grid-cols-4 gap-3">
        {[
          ...
          <button key={a.label} onClick={a.action} className="bg-white border border-slate-200 hover:border-teal-300 hover:shadow-sm rounded-xl p-4 flex flex-col items-start gap-2 transition-all text-left">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <h2 className="font-medium text-slate-900">Recent Documents</h2>
          ...
          {docsLoading ? <Spinner /> : docsError ? <ErrorState message={docsError} onRetry={reload} /> : (
            <div className="divide-y divide-slate-100">
              {documents.map((d) => {
                ...
                return (
                  <button key={d._id} onClick={() => openDoc(d._id)} className="w-full flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50 text-left">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-400">{d.category} · {fmtDate(d.createdAt)}</p>
                    </div>
                    {d.aiSummary && <span className="text-[11px] text-teal-700 font-medium bg-teal-50 px-2 py-0.5 rounded-full">AI ready</span>}
                  </button>
                );
              })}
              {documents.length === 0 && <p className="text-sm text-slate-400 text-center py-10">No documents yet — upload your first one.</p>}
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-slate-200">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="font-medium text-slate-900">Upcoming Reminders</h2>
            ...
            <div className="divide-y divide-slate-100">
              ...
              {upcoming.length === 0 && <p className="text-sm text-slate-400 text-center py-8">Nothing scheduled.</p>}
            </div>
          </div>
