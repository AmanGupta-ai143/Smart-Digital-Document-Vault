...
function ContactCard({ contact, onCall }) {
  return (
    <div className="bg-white border border-slate-200 hover:border-teal-200 rounded-xl p-4 flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-slate-400 truncate">{contact.phoneNumber} · {contact.category}</p>
      </div>
      <button onClick={() => onCall(contact.name)} className="p-2 rounded-full bg-teal-50 text-teal-700 hover:bg-teal-100 shrink-0"><Phone size={15} /></button>
    </div>
  );
}
  ...
  return (
    <div className="p-5 md:p-8 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="font-serif text-2xl text-slate-900">Important Contacts</h1>
          <p className="text-slate-500 text-sm mt-1">Keep your essential contacts securely organized and easy to access.</p>
        </div>
      ...
      {loading ? <Spinner label="Loading contacts…" /> : error ? <ErrorState message={error} onRetry={reload} /> : (
        <>
                ...
                {emergency.map((c) => (
                  <div key={c._id} className="bg-rose-50 border border-rose-100 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-500 truncate">{c.category}</p>
                    </div>
          ...
          <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1">
            {tabs.map((t) => (
              <button key={t} onClick={() => setTab(t)} className={`px-3.5 py-1.5 rounded-full text-sm font-medium whitespace-nowrap ${tab === t ? "bg-slate-900 text-white" : "bg-white border border-slate-200 text-slate-600"}`}>
                {t === "all" ? "All" : t[0].toUpperCase() + t.slice(1)}
              </button>
            ))}
          </div>

          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 mb-5 max-w-sm">
            <Search size={15} className="text-slate-400" />
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search contacts…" className="text-sm outline-none flex-1" />
          </div>
