  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-12 pb-20 grid md:grid-cols-2 gap-12 items-center">
        <div>
          <span className="inline-block text-xs font-medium tracking-wide uppercase text-teal-700 bg-teal-50 px-3 py-1 rounded-full mb-5 dark:bg-teal-900/40">Your documents, organized by AI</span>
          <h1 className="font-serif text-4xl md:text-5xl leading-[1.1] mb-5">Your personal, AI-powered digital vault</h1>
          <p className="text-slate-600 text-lg mb-8 max-w-md dark:text-slate-400">Securely store documents and contacts, let AI organize and summarize everything, and reach your vault from any authorized device.</p>
          <div className="flex flex-wrap gap-3">
            ...
            <a href="#features" className="border border-slate-300 hover:border-slate-400 text-slate-700 px-6 py-3 rounded-lg font-medium transition-colors dark:text-slate-300">Explore Features</a>
          </div>
          <p className="text-xs text-slate-400 mt-6 dark:text-slate-500">Protected by encrypted sessions and per-device authorization.</p>
        </div>
        <div className="relative">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-5 -rotate-2 dark:bg-slate-900 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-4">
              <Seal size={28} tone="amber"><ShieldCheck size={14} /></Seal>
              <div>
                <p className="text-sm font-medium">Health_Insurance_Policy.pdf</p>
                <p className="text-xs text-slate-400 dark:text-slate-500">Uploaded · AI summary ready</p>
              </div>
            </div>
            <p className="text-sm text-slate-600 leading-relaxed border-l-2 border-teal-200 pl-3 dark:text-slate-400">Individual health insurance policy covering hospitalization up to ₹5,00,000 — renewal due soon.</p>
          </div>
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-4 mt-4 ml-10 rotate-1 dark:bg-slate-900 dark:border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500">Emergency contact · Father</p>
              </div>
      ...
      <section id="features" className="max-w-6xl mx-auto px-6 py-16 border-t border-slate-200 dark:border-slate-700">
        <h2 className="font-serif text-2xl mb-10 text-center">Everything your vault needs</h2>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
          {features.map((f) => (
            <div key={f.title} className="bg-white rounded-xl border border-slate-100 p-5 hover:shadow-md transition-shadow dark:bg-slate-900 dark:border-slate-800">
              <div className="w-10 h-10 rounded-lg bg-teal-50 flex items-center justify-center mb-3 dark:bg-teal-900/40">
      ...
      <footer className="border-t border-slate-200 py-8 text-center text-sm text-slate-400 dark:border-slate-700 dark:text-slate-500">
