...
export default function ActivityPage() {
  ...
  return (
    <div className="p-5 md:p-8 max-w-3xl mx-auto">
      <h1 className="font-serif text-2xl text-slate-900 mb-1 dark:text-slate-100">Activity History</h1>
      <p className="text-slate-500 text-sm mb-6 dark:text-slate-400">A record of what's happened in your vault.</p>
      ...
      {loading ? <Spinner label="Loading activity…" /> : error ? <ErrorState message={error} onRetry={load} /> : logs.length === 0 ? (
        ...
        <div className="space-y-6">
          {grouped.map(([day, items]) => (
            <div key={day}>
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-2 dark:text-slate-500">{fmtDate(day)}</p>
              <div className="bg-white border border-slate-200 rounded-xl divide-y divide-slate-100 dark:bg-slate-900 dark:border-slate-700 dark:divide-slate-800">
                {items.map((a) => {
                  ...
                  return (
                    <div key={a._id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <p className="text-sm text-slate-700 flex-1 dark:text-slate-300">{a.description}</p>
                      <span className="text-xs text-slate-400 shrink-0 dark:text-slate-500">{new Date(a.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                    </div>
                  );
                })}
              </div>
