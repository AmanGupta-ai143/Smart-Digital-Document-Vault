...
export default function NotificationsPanel({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-transparent" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="absolute top-16 right-4 md:right-8 w-80 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto">
          ...
          {!loading && notifications.length === 0 && <p className="text-sm text-slate-400 text-center py-8">You're all caught up.</p>}
          {notifications.map((n) => {
            const Icon = ICON[n.category] || Bell;
            return (
              <button key={n._id} onClick={() => markRead(n._id)} className={`w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-slate-50 ${n.isRead ? "opacity-60" : ""}`}>
