...
import { getTimeBasedGreeting } from "../lib/greetings.js";
...
export default function Dashboard({ setPage, openDoc, openUpload }) {
  ...
  // Recomputed only when firstName changes (i.e. once per session), so the
  // greeting stays put during a visit but picks a fresh variant next time.
  const greeting = useMemo(() => getTimeBasedGreeting(firstName), [firstName]);

  return (
    <div className="p-5 md:p-8 max-w-6xl mx-auto space-y-8">
      <div>
        <h1 className="font-serif text-2xl md:text-3xl text-slate-900 dark:text-slate-100">{greeting}</h1>
